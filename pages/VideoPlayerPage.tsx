...
const VideoPlayerPage: React.FC = () => {
        ...
        .then((details) => {
          if (isMounted) {
            ...
            if (
              initialVideo &&
              initialVideo.views &&
              (details.views === '0回視聴' || details.views === '視聴回数不明' || details.views === '0回' || details.views.startsWith('0'))
            ) {
  ...
  const applyPlaybackSettings = (speed: number, transpose: number) => {
    ...
    if (defaultPlayerMode === 'player' && iframeRef.current && iframeRef.current.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(JSON.stringify({ event: 'command', func: 'setPlaybackRate', args: [targetSpeed] }), '*');
    }
  ...
  const commentCountDisplay = videoDetails.commentCount
    ? videoDetails.commentCount + '件のコメント'
    : comments.length > 0
    ? `${comments.length.toLocaleString()}件以上のコメント`
    : 'コメント';
    ...
    <div className="flex flex-col lg:flex-row gap-6 max-w-[1750px] mx-auto pt-2 md:pt-6 px-4 md:px-6 justify-center">
      <div className="flex-1 min-w-0 max-w-full">
            ...
            <div className="w-full h-full flex items-center justify-center text-white bg-black">
              {isStreamDataLoading ? (
                ...
                <div className="text-center">
                  ...
                  <button onClick={fetchStreamDataIfNeeded} className="mt-2 text-blue-400 hover:underline">
                    再試行
                  </button>
                </div>
              )}
        ...
        <div className="">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-2">
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-shrink-0 relative">
              ...
              <button
                ...
                className={`flex items-center justify-center rounded-full w-9 h-9 transition-colors flex-shrink-0 ${
                  showLiveChat
                    ? 'bg-yt-light dark:bg-[#272727] text-yt-blue'
                    : 'bg-yt-light dark:bg-[#272727] text-black dark:text-white hover:bg-[#e5e5e5] dark:hover:bg-[#3f3f3f]'
                }`}
                title="ライブチャット表示"
              >
                  ...
                  <div className="absolute bottom-full right-0 mb-2 w-72 bg-yt-white dark:bg-yt-light-black rounded-xl shadow-xl border border-yt-spec-light-20 dark:border-yt-spec-20 p-4 z-50 animate-scale-in">
                    ...
                    <div className="mb-6">
                      ...
                      <input
                        ...
                        className="w-full accent-yt-blue h-1 bg-yt-light-gray rounded-lg appearance-none cursor-pointer"
                      />
                    </div>
                  </div>
                )}
          ...
          <div
            className={`mt-4 bg-yt-spec-light-10 dark:bg-yt-dark-gray p-3 rounded-xl text-sm cursor-pointer hover:bg-yt-spec-light-20 dark:hover:bg-yt-gray transition-colors ${
              isDescriptionExpanded ? '' : 'h-24 overflow-hidden relative'
            }`}
                    ...
                    <div className="flex gap-1 ml-4">
                      <button
                        ...
                        className={`px-3 py-1 text-sm font-semibold rounded-full transition-colors ${
                          commentSort === 'top'
                            ? 'bg-black text-white dark:bg-white dark:text-black'
                            : 'text-yt-light-gray hover:bg-yt-spec-light-10 dark:hover:bg-yt-spec-10'
                        }`}
                      >
                        おすすめ順
                      </button>
                ...
                {isCommentsLoading ? (
                  ...
                  <div className="space-y-4">
                    {comments.map((comment, idx) => (
                      <CommentComponent key={`${comment.comment_id}-${idx}`} comment={comment} />
                    ))}
      ...
      <div className="w-full lg:w-[350px] xl:w-[400px] flex-shrink-0 flex flex-col gap-4 pb-10">
        ...
        {showLiveChat && (
          <div className="w-full h-[600px] bg-yt-white dark:bg-yt-light-black rounded-xl overflow-hidden border border-yt-spec-light-20 dark:border-yt-spec-20 relative hidden lg:block mb-4">
            <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-0">
              <p className="text-xs text-yt-light-gray">読み込み中...</p>
            </div>
            <iframe src={chatSrc} className="w-full h-full relative z-10" frameBorder="0"></iframe>
            <div className="absolute bottom-2 right-2 z-20">
              <button
                onClick={() => window.open(chatSrc, 'LiveChat', 'width=400,height=600')}
                className="text-xs bg-yt-blue text-white px-2 py-1 rounded opacity-70 hover:opacity-100"
              >
                別窓
              </button>
            </div>
        ...
        {currentPlaylist && (
          <PlaylistPanel
            ...
            videos={isShuffle ? shuffledVideos : playlistVideos}
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 pt-0">
          <button className="px-3 py-1.5 bg-black dark:bg-white text-white dark:text-black text-xs md:text-sm font-semibold rounded-lg whitespace-nowrap">
            すべて
          </button>
          <button className="px-3 py-1.5 bg-yt-light dark:bg-[#272727] text-black dark:text-white text-xs md:text-sm font-semibold rounded-lg whitespace-nowrap hover:bg-gray-200 dark:hover:bg-gray-700">
            関連動画
          </button>
        </div>
        ...
        <div className="flex flex-col space-y-3">
          {relatedVideos.length > 0
            ? relatedVideos.map((video, idx) => <RelatedVideoCard key={`${video.id}-${idx}`} video={video} />)
